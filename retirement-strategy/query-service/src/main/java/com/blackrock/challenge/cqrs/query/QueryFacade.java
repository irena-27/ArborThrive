...
import com.blackrock.challenge.store.cache.ReadCacheStore;
...
public class QueryFacade {
  ...
  private final ReadCacheStore readCache;
  ...
  public CompletionStage<ReturnsResponse> returns(Instrument instrument, ReturnsRequest req, String idempotencyKey) {
    ...
    String readCacheKey = "returns:" + instrument.name() + ":" + req.toString();
    Object projection = readCache.getIfPresent(readCacheKey);
    if (projection instanceof ReturnsResponse rr2) {
      return CompletableFuture.completedFuture(rr2);
    }
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      readCache.put(readCacheKey, response, 300);
      return response;
    }, returnsExecutor);
  }

  public PerformanceResponse performance() {
    Object perfCached = readCache.getIfPresent("performance:last");
    if (perfCached instanceof PerformanceResponse pr) return pr;
    ...
    PerformanceResponse resp = new PerformanceResponse(time, memory, threads);
    readCache.put("performance:last", resp, 3);
    return resp;
  }
}
